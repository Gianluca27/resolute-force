import type { TestimonialsProps, SectionStyle } from '@resolute/shared';
import { sectionOverrides, SECTION_PAD, SECTION_PX } from './shell';

export default function Testimonials({ id, props, sectionStyle }: { id: string; props: TestimonialsProps; sectionStyle?: SectionStyle }) {
  const o = sectionOverrides(sectionStyle, { bg: 'bg-panel border-y border-line', pad: SECTION_PAD });
  if (props.items.length === 0) return null;
  return (
    <section id={id} data-screen-label="Testimonios" style={o.css} className={`scroll-mt-20 ${o.bgCls} ${SECTION_PX} ${o.padCls}`}>
      <div className="max-w-[1180px] mx-auto">
        ...
        {props.title && <h2 className="m-0 mb-10 font-display font-black uppercase leading-[0.92] tracking-[-0.01em] text-[clamp(2rem,4.5vw,3.2rem)]">{props.title}</h2>}
