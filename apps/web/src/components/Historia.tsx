import type { HistoriaProps, SectionStyle, SectionLayout } from '@resolute/shared';
...
export default function Historia({ id = 'historia', props, sectionStyle, layout }: { id?: string; props: HistoriaProps; sectionStyle?: SectionStyle; layout?: SectionLayout }) {
    ...
    <section id={id} data-screen-label="Historia" style={o.css} className={`scroll-mt-20 ${o.bgCls} ${SECTION_PX} ${o.padCls}`}>
      <div className="max-w-[1180px] mx-auto flex flex-wrap gap-[clamp(36px,5vw,72px)] items-center">
        <div className="flex-1 basis-[400px] min-w-[300px] order-2">
          ...
          <h2 {...el('title', 'm-0 font-display font-black uppercase leading-[0.92] tracking-[-0.01em] text-[clamp(2.3rem,5vw,4rem)]')}>
          ...
          {props.stats.length > 0 && (
            <div {...el('stats', 'flex flex-wrap gap-[14px] mt-[34px]')}>
        ...
        <div className="flex-1 basis-[360px] min-w-[300px] order-1">
          <div {...el('image', 'border border-line2 rounded-[var(--rf-radius)] overflow-hidden relative')}>
