import type { HeroProps, SectionStyle, SectionLayout } from '@resolute/shared';
...
export default function Hero({ id = 'hero', props, sectionStyle, layout, anchors }: {
  id?: string;
  props: HeroProps;
  ...
  return (
    <section id={id} data-screen-label="Hero" style={o.css} className={`relative scroll-mt-20 min-h-[88vh] flex items-center overflow-hidden ${SECTION_PX} ${o.padCls} ${o.bgCls}`}>
      ...
      <div className="relative z-[2] max-w-[1180px] mx-auto w-full">
        {props.kicker && (
          <div {...el('kicker', 'inline-flex items-center gap-3 mb-[26px]')}>
        ...
        <h1 className="m-0 font-display font-black uppercase leading-[0.82] tracking-[-0.02em] text-[clamp(3.6rem,11vw,9.5rem)]">
          <span {...el('title1', 'block text-tx')}>{props.title1}</span>
          <span {...el('title2', 'block text-red', { textShadow: '0 0 60px rgb(var(--rf-accent) / .35)' })}>{props.title2}</span>
        </h1>
        <p {...el('subtitle', 'max-w-[540px] mt-[30px] text-mut leading-[1.6] text-[clamp(16px,1.3vw,19px)]')}>{props.subtitle}{props.subtitleHighlight && <> <span className="text-tx font-semibold">{props.subtitleHighlight}</span></>}</p>
        <div className="flex flex-wrap gap-[14px] mt-[38px]">
          {props.ctaPrimary && (
            <a href={`#${anchors?.products ?? 'productos'}`} {...el('ctaPrimary', 'inline-flex items-center gap-[10px] bg-red text-white no-underline font-display font-bold text-[17px] tracking-[0.13em] uppercase px-[34px] py-[17px] rounded-[var(--rf-btn-radius)] transition hover:bg-redd hover:-translate-y-[2px]')}>{props.ctaPrimary}
