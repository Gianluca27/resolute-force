...
export default function Footer({ contactWhatsapp, contactInstagram }: { contactWhatsapp?: string; contactInstagram?: string }) {
  const ig = `https://www.instagram.com/${(contactInstagram ?? '@resoluteforceok').replace('@', '')}/`;
  return (
      ...
      <div className="max-w-[1180px] mx-auto flex flex-wrap gap-10 justify-between">
        ...
        <div className="flex gap-[clamp(34px,6vw,80px)] flex-wrap">
          ...
          <div>
            ...
            <div className="flex flex-col gap-[11px]"><a href={ig} target="_blank" rel="noopener" className={fl}>Instagram</a><a href={`https://wa.me/${contactWhatsapp ?? '5493413213723'}`} target="_blank" rel="noopener" className={fl}>WhatsApp</a></div>
          </div>
        </div>
      </div>
