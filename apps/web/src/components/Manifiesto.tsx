import type { ManifiestoProps, SectionStyle, SectionLayout } from '@resolute/shared';
...
export default function Manifiesto({ id = 'manifiesto', props, sectionStyle, layout }: { id?: string; props: ManifiestoProps; sectionStyle?: SectionStyle; layout?: SectionLayout }) {
    ...
    <section id={id} data-screen-label="Manifiesto" style={o.css} className={`scroll-mt-20 ${o.bgCls} ${SECTION_PX} ${o.padCls}`}>
      <div className="max-w-[1180px] mx-auto flex flex-wrap gap-[clamp(36px,5vw,72px)] items-center">
        <div className="flex-1 basis-[360px] min-w-[300px] relative">
          ...
          <div {...el('image', 'relative border border-line2 rounded-[var(--rf-radius)] overflow-hidden bg-black')}>
        ...
        <div className="flex-1 basis-[420px] min-w-[300px]">
          ...
          <h2 {...el('title', 'm-0 font-display font-black uppercase leading-[0.92] tracking-[-0.01em] text-[clamp(2.4rem,5.4vw,4.4rem)]')}>{props.title}{props.titleAccent && <><br /><span className="text-red">{props.titleAccent}</span></>}</h2>
          <p {...el('body', 'mt-6 text-mut leading-[1.7] max-w-[520px] text-[clamp(16px,1.2vw,18px)]')}>{props.body}{props.bodyHighlight && <> <span className="text-tx font-semibold">{props.bodyHighlight}</span></>}</p>
          {props.principles.length > 0 && (
            <div {...el('principles', 'flex flex-col mt-9 border-b border-line')}>
